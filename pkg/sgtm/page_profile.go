...
func (svc *Service) profilePage(box *packr.Box) func(w http.ResponseWriter, r *http.Request) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// load profile
		{
			...
			user, err := svc.store.GetUserBySlug(userSlug)
			if err != nil {
		...
		// tracks
		{
			data.Profile.LastTracks, data.Profile.Stats.Tracks, err = svc.store.GetPostListByUserID(data.Profile.User.ID, 100)
			if err != nil {
		...
		if data.Profile.Stats.Tracks > 0 {
			timestamps, err := svc.store.GetCalendarHeatMap(data.Profile.User.ID)
			if err != nil {
